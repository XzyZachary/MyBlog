...
<z-container>
    <div class="search">
        <el-input placeholder="请输入内容" prefix-icon="el-icon-search"
         v-model="keyword" @keydown.enter.native="getBlogList"></el-input>
        <el-button type="primary" icon="el-icon-search" :loading="loading" 
        @click="getBlogList">搜索</el-button>
    </div>
    ...
    stripe border>
        ...
        && item.filters" :key="index" :label="item.label" 
        :prop="item.prop" :header-align="item.headerAlign" 
        :align="item.align" :sortable="item.sort" 
        ...
        :min-width="item.minWidth || 200">
            <template slot-scope="scope">
                <el-tag
                    ...
                    type="primary"
                    close-transition 
                    v-for="(tag, index) in scope.row.type" :key="index">
                    {{tag}}</el-tag>
            </template>
        </el-table-column>
...
<script>
    ...
    // import EditComponent from '../edit/index' import { blogFilters } from 'store/modules/classify'
    export default {
